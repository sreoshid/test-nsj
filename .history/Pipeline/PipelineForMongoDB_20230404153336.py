...
from MongoDBqueries import find_latest_createDate
from MongoDBqueries import find_oldest_createDate
...
def getRollbackDateFromUser():
    rollback_date = input("Enter a date in YYYY-MM-DD format: ")
    try:
        date = datetime.strptime(rollback_date, "%Y-%m-%d")
        latestCreateDate = find_latest_createDate(code)oldestCreateDate = find_oldest_createDate(code)
